...
void Props_Chair_Animate(gentity_t * ent)
{
	...
	if(!Q_stricmp(ent->classname, "props_chair"))
	{
		...
		else
		{
			...
			(!Q_stricmp(ent->classname, "props_chair_chatarm")) || (!Q_stricmp(ent->classname, "props_chair_hiback")))
	{
		if(ent->s.frame >= 20)
		{
			ent->s.frame = 20;
...
void props_castlebed_touch(gentity_t * ent, gentity_t * other, trace_t * trace)
{
	...
	if(other->client->ps.pm_flags & PMF_JUMP_HELD && other->s.groundEntityNum == ent->s.number && !other->client->ps.pm_time)
	{
...
//----(SA)  modified
	...
	trap_SetConfigstring(CS_SKYBOXORG,
						 va("%.2f %.2f %.2f %.1f %i %.2f %.2f %.2f %i %i", ent->s.origin[0], ent->s.origin[1], ent->s.origin[2],
							fov_x, (int)isfog, fogv[0], fogv[1], fogv[2], fogn, fogf));
//----(SA)  end
}
