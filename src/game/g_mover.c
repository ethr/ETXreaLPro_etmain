...
//      trap_LinkEntity( ent );
	...
	{							// RF, added this for bats, but this is safe for all movers, since if they aren't solid, and aren't visible to the client, they don't need to be linked
		trap_LinkEntity(ent);
		...
		else if(ent->key || ent->allowteams)
		{						// doors should only block when locked and at POS1
			switch (moverState)
			{
				...
				case MOVER_POS1:
					...
					if(ent->allowteams)
					{
						...
						if(!blockflags)
						{
							...
							{
								G_Printf
									("WARNING: %s at %s has allowteams flag, but no spawnflags to define which team is allowed\n",
...
// XreaL BEGIN
		...
		if((ent->targetname && !ent->targetnameAutogenerated) || ent->takedamage)
...
// XreaL BEGIN
	// Doom 3 mapping convention: every entity has a name
	else if(ent->targetname && !ent->targetnameAutogenerated)
	{
...
// XreaL BEGIN
	// Doom 3 mapping convention: every entity has a name
	if((ent->targetname && !ent->targetnameAutogenerated) && level.numBrushModels < 128)
// XreaL END
	{
